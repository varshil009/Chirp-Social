...
function useTweetLike(queryKey = ['tweets', {}]) {
  ...
  return useMutation(
    (tweetId) => client.post(`/tweets/like/${tweetId}`).then((res) => res.data),
    {
      onMutate: async (tweetId) => {
        await queryClient.cancelQueries(queryKey);
        ...
        queryClient.setQueryData(queryKey, (old) => {
          if (old?.tweet) {
            ...
            return {
              ...
              tweet: {
                ...old.tweet,
                likes: [...(old.tweet.likes || []), authUser?._id].filter(Boolean),
              },
            };
          } else if (old?.pages) {
            return {
              ...
              pages: old.pages.map((page) => ({
                ...
                results: page.results.map((tweet) => {
                  ...
                  return {
                    ...tweet,
                    likes: [...(tweet.likes || []), authUser?._id].filter(Boolean),
                  };
                }),
              })),
            };
...
function useTweetUnlike(queryKey = ['tweets', {}]) {
  ...
  return useMutation(
    (tweetId) => client.delete(`/tweets/like/${tweetId}`).then((res) => res.data),
    {
      onMutate: async (tweetId) => {
        await queryClient.cancelQueries(queryKey);
